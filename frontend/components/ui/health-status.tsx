    ...
    return (
      <div className="space-y-3">
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
          No se pudo leer el estado del backend: {state.message}
        </div>
        {mode === "technical" ? (
          <div className="rounded-2xl border border-black/5 bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">API configurada</p>
            <p className="mt-2 break-all text-sm font-medium text-brand-ink">{getConfiguredApiUrl()}</p>
          </div>
        ) : null}
      </div>
    );
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <div className="rounded-2xl border border-black/5 bg-white p-4">
        ...
        <p className="mt-2 text-lg font-semibold text-brand-ink">{state.data.status}</p>
      </div>
      <div className="rounded-2xl border border-black/5 bg-white p-4">
      ...
      <div className="rounded-2xl border border-black/5 bg-white p-4">
        ...
        <p className="mt-2 text-lg font-semibold text-brand-ink">{state.data.project}</p>
      </div>
    </div>
  );
}
